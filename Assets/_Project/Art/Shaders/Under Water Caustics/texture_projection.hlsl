float2 Unity_Rotate_Degrees_float(float2 UV, float Center, float Rotation)
{
    Rotation = Rotation * (3.1415926f / 180.0f);
    UV -= Center;
    float sine = sin(Rotation);
    float cosine = cos(Rotation);
    float2x2 rMatrix = float2x2(cosine, -sine, sine, cosine);
    rMatrix *= 0.5;
    rMatrix += 0.5;
    rMatrix = rMatrix * 2 - 1;
...
void TextureProjection_float(in Texture2D Tex, in SamplerState SS, in float3 Position, in float3 Normal, in float Tile, in float Blend, in float Speed, in float Rotation, out float4 Out)
{
